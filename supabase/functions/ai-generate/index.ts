// ai-generate: vendor-neutral, server-side Gemini gateway.
//
...
//
// Testable core: the request pipeline lives in `handleRequest(req, deps)` with
// injectable env/fetch/id/clock, so unit tests exercise it with a fake fetch
// and no network. `Deno.serve` at the bottom wires the real environment.
...
export async function handleRequest(
  ...
  const fail = (code: ErrorCode, event: string): Response =>
    respond(new ApiError(code, code).status, {
...
// ---------------------------------------------------------------------------
// Real runtime wiring. Not exercised by unit tests (which call handleRequest
// with fakes). Deploy is a later, explicit step.
//
...
// They hook in right after CORS/method checks in handleRequest.
// ---------------------------------------------------------------------------
declare const Deno:
  | { env: { get: (k: string) => string | undefined }; serve: (h: (req: Request) => Promise<Response>) => void }
  | undefined;
